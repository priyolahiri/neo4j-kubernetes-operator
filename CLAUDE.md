...
79. **`Neo4jEnterpriseCluster.spec.backups` is deprecated; runtime signal lives in the cluster controller**: Go field marked `// Deprecated:`. When set, the controller emits Warning event `LegacyBackupsDeprecated` and logs an info-level migration pointer each reconcile. The centralized `{cluster}-backup-0` StatefulSet still builds for back-compat. Internal call sites (cluster.go `BuildBackupStatefulSet`, `buildBackupVolumeClaimTemplates`, `getServiceAccountNameForEnterprise`; rolling_upgrade.go; storage_expansion.go; cloud_validator.go) carry `//nolint:staticcheck // SA1019: legacy spec.backups back-compat path.` annotations — keep those when touching the legacy plumbing so the deprecation warning continues to fire only for *new* external callers. Removal target: a future major release; until then, Neo4jBackup CRD is the canonical path.
