...
11. **Operator's outbound Bolt URI uses the routing scheme**: `buildConnectionURIFor{Enterprise,Standalone}` MUST emit `neo4j://` / `neo4j+s://`, never `bolt://` / `bolt+s://`. The Go driver only honors `AccessMode: AccessModeWrite` under the routing scheme; under `bolt://` the access mode is silently ignored and connections produce `Neo.ClientError.Cluster.NotALeader` on N-1/N reconciles. Only legitimate `bolt://` consumer is `splitbrain_detector.go` (bypasses routing on purpose). Pinned by `internal/neo4j/uri_test.go`.
...
42. **`source.type: backup` resolved upstream via `resolveRestoreSource`**: `createRestoreJob` calls it once, swaps `Spec.Source` onto a shallow restore copy, threads the resolved view through every builder. `buildRestoreCommand`'s `case "backup":` is dead-code with a defensive `internal:` error.
...
44. **Restore `--from-path` is a FILE via shell substitution; `tail -1` picks the LATEST**: `buildLocalRestoreFilePath` emits `$(ls '<backupPath>'/'<dbname>'-*.backup | tail -1)`. ISO-8601 timestamps sort lexicographically into chronological order, so `tail -1` returns the latest run in the shared dir (rule 40). **BOTH path AND database name MUST go through `shellQuote()`** — they come from user-controlled `spec.source.backupPath` and `spec.databaseName`; unquoted values like `foo; rm -rf /data #` would let the shell escape the `ls` and execute arbitrary commands in the restore Pod (mounts `/data` RW, carries `NEO4J_ADMIN_PASSWORD`). Pinned by `TestResolveLocalPVCFromPath_BackupPathShellInjectionGuard` + `_NestedCommandSubstitutionGuard` + `_EmbeddedSingleQuoteGuard`. Cloud URIs (`s3://`, `gs://`, `azb://`) skip — native cloud readers handle per-file selection. Never pass the directory; never substitute the timestamp in Go; never drop quoting; never revert to `head -1`.
...
47. **Legacy post-restore re-seed via `dbms.[cluster.]recreateDatabase`** (Job-based standalone path only; rule 75's Cypher path doesn't need it). `recreateRestoredDatabaseOnCluster` calls the recreate procedure with server-0 as the explicit seed (matched by `cluster.Name + "-server-0"` against `SHOW SERVERS YIELD address` — `name` column is unreliable). Procedure name from `version.RecreateDatabaseProcedure()`: `dbms.cluster.recreateDatabase` (5.24–2025.03) → `dbms.recreateDatabase` (2025.04+). Skipped for standalone, `Topology.Servers < 2`, and versions pre-5.24 SemVer / pre-2025.02 CalVer. Non-fatal — failure emits a Warning event `DatabaseCreateFailed` without blocking the restore.
48. **Sharded-DB backup uses `{name}*` glob + always-quoted db arg**: one `neo4j-admin database backup "{name}*" …` invocation captures every shard in a consistent snapshot. `GetBackupCommand` ALWAYS double-quotes the database-name arg so the shell can't pre-expand `*` — pinned by `TestGetBackupCommandQuotesShardedGlob` + `TestGetBackupCommandQuotesPlainName`. `shardedPreflightGlobSafety` rejects any DB matching `{name}*` outside `^{name}-(g|p)\d{3}$` (e.g. `products` backup with colliding `productsales` DB → terminal Failed). `shardedPreflightStatic` routes missing-Ready to Pending (not Failed) so the backup waits for the sharded DB.
...
71. **PVC seed proxy + `URLConnectionSeedProvider`**: PVC-backed `seedBackupRef` (sharded) and PVC-backed cluster Neo4jRestore both spawn a `backup-seed-proxy-<owner>` Deployment + Service via the generic `ensurePVCSeedProxyResources` (`internal/controller/pvc_seed_proxy.go`) — mounts the backup PVC RO, busybox httpd on `:8080`, owner-ref on the consuming CR for GC. URLs target the exact `.backup` filename per shard / database (read from `BackupRun.ShardArtifacts[].Filename` / `BackupRun.ArtifactFilename`); `URLConnectionSeedProvider` only supports single-file URIs, not directory scans. Hard-gated on F3 filename capture — empty filename → validator rejects with actionable error pointing user at re-running the backup (CloudSeedProvider has no such dependency).
72. **ResolvedShardedSeed.URI vs PerShardURIs is mutually exclusive**: cloud paths populate URI (consumed by `OPTIONS { seedURI: '…' }`); PVC paths populate PerShardURIs (consumed by `OPTIONS { seedURIs: { … } }`). When wiring to `shardedDatabase.Spec.SeedURI/SeedURIs`, set ONE and clear the OTHER — validator rejects both set together. `ProxyAvailable` false → requeue while the proxy Deployment rolls out.
74. **`dbms.databases.seed_from_uri_providers` is version-gated** via `SeedFromURIProvidersConfigValue(imageTag)`. Base: `CloudSeedProvider,FileSeedProvider,URLConnectionSeedProvider`. `ServerSeedProvider` appended only on `IsNeo4jVersion202604OrHigher` (class not in META-INF/services pre-2026.04 → "provider not found" warnings if listed). Both cluster (`BuildConfigMapForEnterprise`) and standalone configLines builder call the helper — never inline. **Deprecated `S3SeedProvider` excluded across all versions**: `CloudSeedProvider` handles `s3://` via the SDK default credential chain (envFrom; rule 59), making legacy `seedCredentials` Cypher option unnecessary. Users can opt back in via spec.config if needed. Pinned by `TestSeedFromURIProvidersConfigValue` + `TestIsNeo4jVersion202604OrHigher`.
...
76. **seedURI for cluster restore is a DIRECTORY (cloud) or file URL (PVC)**: cloud → `buildSeedURIFromBackupStorage` returns `<scheme>://<bucket>/<path>/<chain-root>/` (trailing slash mandatory — no-slash = single artifact path), `CloudSeedProvider` scans + applies chain. PVC → `resolveClusterPVCRestoreURI` spawns the seed proxy (rule 71) and emits an http URL pointing at `BackupRun.ArtifactFilename`. Standalone Job path uses `tail -1` of timestamped glob (rule 44).

77. **`RecreateDatabaseWithSeedURI` vs `RecreateDatabase` (seedingServers)**: distinct methods on `internal/neo4j/client.go`. `RecreateDatabaseWithSeedURI` is the cluster-native restore primitive — every server pulls from the URI in parallel, no Job. `RecreateDatabase` (with `seedingServerIDs`) is a post-`neo4j-admin restore` consistency fix that picks server-0 as the seed — legacy/standalone only. Never use seedingServers where seedURI works.
78. **`spec.chainFromBackup` composes mixed-cadence FULL+DIFF**: a daily FULL CR + hourly DIFF CR with `chainFromBackup: daily` share `<base>/<chain-root>/` so `neo4j-admin --type=DIFF` finds the prior FULL. `chainRoot(backup)` returns `spec.chainFromBackup` if set, else `backup.Name`; used by `buildToPath`, `BackupRun.BackupsPath`, and the `app.kubernetes.io/part-of` Job label. Validator rejects self-reference; reconciler's `validateChainParent` enforces parent existence + matching target + matching storage. `waitForChainConcurrencyClear` lists Jobs with `part-of=<chain-root>` and refuses to start while any is `status.active>0` — concurrent chained Jobs corrupt neo4j-admin's chain detection. Routes `errChainBusy` to Pending+requeue, NOT terminal Failed. Pinned by `test/integration/backup_chain_test.go`.
