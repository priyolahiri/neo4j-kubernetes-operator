...
| **Composite databases** (`Neo4jCompositeDatabase`) | Standalone | A composite stores no data and has no topology, so clustering adds nothing to its code path — it is alias DDL plus an ordering guard. A **remote** constituent would need a second DBMS; pointing one back at the same server proves the keystore path without a second Enterprise JVM, so it stays here too |
...
| Composite database | `Neo4jCompositeDatabase` with two local constituents → Ready; `status.observedConstituents` lists them **fully qualified** (`cineasts.latest`), read back from the server rather than echoed from spec |
| Composite: where a constituent is reachable from | from a session **on the composite** (`cypher-shell -d cineasts`), `USE cineasts.latest MATCH (n) RETURN count(n)` succeeds and `RETURN graph.names()` lists the constituents. From the default database the same `USE` is refused with `42N04` — **and following that error's own advice to "connect to `cineasts.latest` directly" fails with `22N51`**. Both halves matter: the guide tells users to ignore the server's suggestion, so a release that changed this would make the docs wrong |
| Composite: the ordering trap is guarded | create `CREATE ALIAS cineasts.latest` by hand with no composite present, then apply a `Neo4jCompositeDatabase` named `cineasts`: the CR reports `Failed` with reason `CompositeDatabaseNameBlocked` and **names the alias to drop**. Drop it → the composite is created on the next reconcile. Neo4j's own `42N87` never mentions ordering, so the operator's message is the only route out |
| Composite: constituents are enforced | with `enforceConstituents: true` (the default), create an extra alias in the composite's namespace by hand → next reconcile removes it. Set `false` → a hand-made constituent survives and appears in `observedConstituents` |
| Composite: a GRAPH privilege on it is caught | a `Neo4jRole` granting `MATCH {*} ON GRAPH <composite>` → `PrivilegesResolve=False` with reason `GraphPrivilegeOnComposite`. Neo4j accepts and persists such a grant while it does nothing, so the operator's warning is the only signal |
| Composite: deletion removes aliases, not data | `deletionPolicy: Delete` → `DROP COMPOSITE DATABASE ... CASCADE ALIASES`; afterwards `SHOW ALIASES` has none of them and **both target databases are still online with their data** |
| Composite: remote constituent keystore | set `spec.remoteAliasKeystore` on the standalone, then a constituent with `remote.credentialsSecretRef` whose `url` points back at **this same DBMS** — enough to prove the keystore mounts and Neo4j can encrypt the alias credentials, without a second Enterprise JVM. Omitting the keystore must be refused **at apply time** naming the field, not inside the server with a `50N09`/`50N00` that mentions neither the CR nor the constituent |
...
| CLI: `cypher` never moves the password | `kubectl neo4j cypher <name> -c "SHOW DATABASES"` returns the same rows as the by-hand `kubectl exec`. **`connect` prints no exec line at all** — it points at `kubectl neo4j cypher` and says the credentials are already in the pod; it is `cypher` that execs, referencing `$DB_USERNAME` / `$DB_PASSWORD` **by name** (`connect.go`), so the value reaches neither your shell history nor the API audit log. Check by grepping `connect`'s whole output for the real admin password: zero hits |
...
| Cross-topology restore | restore the **retained standalone backup** from Phase 1 into the **cluster** via `instanceRef`; confirm the data round-trips. The cluster path stands up a short-lived `backup-seed-proxy-<restore>` Deployment that serves the backup PVC over HTTP. **The seed URI is not in `status`** — `Neo4jRestore.status` has no such field; read it from the `RestoreStarted` event (`kubectl get events -n <ns> --field-selector reason=RestoreStarted`), which carries `seedURI=http://backup-seed-proxy-<restore>.<ns>.svc.cluster.local:8080/<path>/<artifact>.backup` — an `http://` URL, not a `file://` path. The proxy Deployment appearing and then being reaped is the other half of the evidence |
...
`crossClusterReplication`, no LoadBalancer needed. Part D smoke-tests the
`crossClusterReplication` toggle on a single plain Kind cluster, where the
proxy's Service never gets an address. **Part E is the one that matters most**:
two Kind clusters, TLS on with mutual CA trust, the proxy holding a real
MetalLB address — the configuration CCDR is actually sold with, and the only
one that exercises the proxy, the certificate SANs, the advertised-address
override and cross-cluster trust together. See each part for what it does and
does not cover.
...
| A second writer to the same path is refused | while ANOTHER `Neo4jBackup` targets the same bucket + `storage.path`, the chain refuses to run and says why — *"a second writer can interleave artifacts and break the differential chain a replica depends on"* — pointing at `storage.path` or `spec.chainFromBackup`. Easy to trigger by accident: the rejected CRs from the row above collide with the good one unless they use a different path, and the chain stays stuck until they are deleted |
| A FULL then a DIFF | let (or force) two runs so the bucket holds a full **and** a differential — a seed-only chain does not exercise the pull path. **`status.history[]` does not record the backup type**, so read it from the run's Job log (`Finished full backup … Downloaded from tx -1 to tx N` then `Finished differential backup … from tx N+1`) or from the artifact sizes in the bucket. Run 1 logs *"Start differential … Falling back to full"* — correct, there is no parent yet. Write to the CHAIN'S database between runs, not the default one: `kubectl neo4j cypher` has no `-d` flag, so use `kubectl exec … cypher-shell -d <db>` or the diff has nothing to carry |
...
state, which a `type: LoadBalancer` Service never gets on *plain* Kind.
**`make ccdr-lb` now supplies it** (`hack/metallb-setup.sh` — MetalLB L2 with a
pool carved from the top of the shared `kind` Docker network), so the proxy
path is verifiable locally, and Part E below walks it end to end across two
Kind clusters. This part stays useful as the single-cluster smoke check: the
toggle must never break normal cluster formation, whether or not a real load
balancer is present to service it. Run it **without** `make ccdr-lb` — the
rows below describe the no-address state on purpose.
...
| Advertised address stays internal until ready | `status.crossClusterReplication` stays empty (Kind has no LoadBalancer controller, so the Service never gets an ingress address) → the EFFECTIVE config in the pod still advertises the pod itself: `server.cluster.advertised_address=<pod FQDN>:6000`, not the proxy's hostname. **Do not hardcode the path — ask the process**, because it has moved three times: `kubectl exec <pod> -c neo4j -- sh -c 'CONF=$(ps aux | grep -o "config-dir=[^ ]*" | head -1 | cut -d= -f2); echo $CONF; grep ^server.cluster.advertised_address $CONF/neo4j.conf'`. The ConfigMap never holds advertised addresses at all (it says so: *"advertised addresses will be set dynamically by startup script"*) — `buildStartupScriptForEnterprise` substitutes them at pod start, overriding 6000 only once `status.crossClusterReplication.LoadBalancerHostname` is set. This row named the ConfigMap until 2026-09-12, `/var/lib/neo4j/conf` until 2026-09-13, then `/tmp/neo4j-config` until 2026-09-23 — when the env-var config fix (99e60e1/0cb2ee8) stopped the startup script pointing `NEO4J_CONF` at its own directory, so `--config-dir` went back to `/var/lib/neo4j/conf` and `/tmp/neo4j-config` no longer exists. A grep of an absent path returns nothing, which reads exactly like the override failing |
...
**Part E — the whole proxy path, two Kind clusters, TLS ON (two concurrent deployments):**

This is the configuration CCDR is actually sold with, and the only part that
exercises the proxy, the certificate SANs, the advertised-address override and
cross-cluster CA trust *together*. All of that was verified for the first time
on 2026-09-14, and three defects fell out of it that nothing else had reached
...
| Scenario | Verify |
|---|---|
| CA exchange really crossed | copy each cluster's `ca.crt` out of its `<name>-tls-secret` into a Secret on the *other* cluster and reference it from `spec.tls.additionalClusterTrustCAs`. Then check it landed: `md5sum /ssl/trusted/*.crt` in each `server-0` — the upstream's `peer-ca-0.crt` must equal the downstream's own `ca.crt`, and vice versa. Both clusters must still reach `Ready`; a peer CA that stops a pod starting is a blocker, not a warning |
| A wedged cluster can still be repaired | while you are here: if a template change does NOT reach a cluster that is stuck mid-formation, that is the bug fixed in 40713d7 regressing. Editing the CR must change the StatefulSet even when no pod is ready |
| The proxy gets a real address | `status.crossClusterReplication.ready: true` with one address per server ordinal (`<ip>:16000`, `:16001`), and `server.cluster.advertised_address=<ip>:16000` in the effective config — find it with the `--config-dir` one-liner in Part D rather than a hardcoded path. RAFT and routing must stay internal; only the tx-shipping port is proxied. The override applies only once the Service has an ingress address, so this is the first place it is observable at all |
...
| Cross-cluster replication | | | | | | ✅ backup mode + network mode mechanism (same-cluster) + the full proxy path across two Kind clusters with TLS on (`make ccdr-lb`, Part E) |
| Database aliases | | ✅ | | | | ✅ (survives promotion) |
| Composite databases | | ✅ (local + the keystore path) | | | | |
...
| CLI: cloud substrate (`preflight` zones / PSA / default class / private LB) | ✅ (all four reachable on Kind) | | | | | |
| Operator build metadata (`neo4j_operator_build_info`) | | ✅ (scrape `/metrics`) | | | | |
...
|---|---|---|---|
| **v1.16.0 release verification** | 2026-09-23 | ✅ Phases 0-3 and all of Phase 5 pass; **eleven findings, nine fixed, two needed no code** | Full journey on the release candidate. **Phase 0** — every offline check, the exit-code contract, both skip taxonomies, and the three new cloud-substrate `preflight` checks on Kind; the new default-StorageClass check found a true finding immediately (Kind's `standard` is `WaitForFirstConsumer` with `allowVolumeExpansion` unset). The PSA check's claim that the operator's default satisfies `restricted` was proved against real admission: default **admitted**, override **rejected**. **Phase 1** — standalone on 5.26.30; composites end to end including all three halves of the reachability trap (`42N04`, and the server's own advice failing with `22N51`), the `42N87` ordering guard, and `GraphPrivilegeOnComposite`; users/roles with a hyphenated CR name and a real login; alias re-point and drift correction; APOC holding Ready across 8 samples; backup→restore with the marker returning; `system` refused; support bundle with **zero hits for both real passwords**. **Phase 2** — 3 servers `Available`/`Enabled`, a 3-primary database, `cypher` routing correctly on a cluster, `diagnose` exit 0 both healthy and mid-restart, restore into a new database, all-databases restore, and **cross-topology restore carrying a marker from a 5.26.30 standalone into a 2026.08.1 cluster** — across a topology *and* a version boundary. **Phase 3** — `2026.06.0`, `propertyShardingReady`, the shard family online with the CR name differing from the logical name, sharded backup by **CR name** capturing all three artifacts, and the logical name correctly refused. **Phase 5** — A/B on MinIO: `replicationPullURI`, all three chain-breaking configs rejected, a real FULL (`tx -1 to 5`) then DIFF (`tx 6 to 6`) chain, `export replica-database` emitting a clean manifest that validates; then the downstream pulled from the bucket **after the upstream was deleted**, all 4 rows arriving, write refused, alias pre-staged against a replica, promotion to `standard`, **the alias surviving with generation still 1** (a write through it succeeded), the `Promoted` CR deleted without dropping the database, and **out-of-band promotion observed rather than corrected**. C and D passed in the dispatched extended suite. **Part E** — two Kind clusters, TLS, mutual CA trust: proxy on `172.18.255.200:16000/:16001`, the IP in the certificate's `ipAddresses`, CA md5s matching both ways, replica `Replicating` 0 behind, and a row written upstream appearing downstream in **10s**. **Findings.** Fixed: the composite constituent lookalike loop (#394); preflight contradicting the backup validator (#395); `validate` ignoring misspelled fields, plus a published example using `spec.authentication` that could never have worked (#396); the typo check skipping kinds that skip offline (#397); **`ccdr-e2e-up` never able to create its second cluster — both bound `127.0.0.1:6443`, so Part E's automation had never run** (#401); the typo check covering only 13 of 27 kinds (#402); an auth-rule spec that deadlocks against the operator it tests (#403). Docs: the `connect` row, the `validate` kind counts, the restore seed URI (in an event, not status), and **the advertised-address row now naming the file it warns against** — this release's env-var fix moved `--config-dir` back to `/var/lib/neo4j/conf` and deleted `/tmp/neo4j-config`, so the row greps an absent path and reads exactly like the override failing. No change needed: `--access-mode` (docs already correct, my error), and `writer=TRUE` on a replica — Neo4j's own column alongside `access=read-only`, with the write functionally **refused**. Phase 4 **not run** — nothing under `internal/aura/` changed. |
...
| _(pre-release, v1.15.0 candidate)_ | 2026-09-03 | ⚠️ Phases 0-3 walked incl. the TLS trio; Phase 5 blocked — **all findings below fixed and re-verified live the same day** | **3 blockers.** (1) **Flags after a positional argument are silently ignored** across `diagnose`/`explain`/`connect`/`cypher`/`preflight`/`export` — stdlib `flag` stops parsing at the first non-flag arg. It fails in the wrong direction: the wrong namespace is queried and reported as "not found". Every documented `export` invocation fails outright; `preflight Neo4jBackup/nightly -n neo4j` and `cypher prod -n neo4j` from the published docs silently target `default`. (2) **`cypher -c` can hang forever** and leaves an orphaned `cypher-shell` inside the database pod: it execs with `-i` and no `--non-interactive`, so the shell runs the query then reads stdin for more. Twelve orphans accumulated across two hangs and the database stopped answering over Bolt *and* HTTP while the pod stayed `Ready` — the anti-wedge signature, reached with a single deployment. (3) **`cypher` is unusable against a cluster**: it dials `bolt://localhost` on one chosen pod instead of routing, and the default `neo4j` database has 1 primary, so 2 of 3 servers answer "Database neo4j not found"; there is no database flag to work around it. **Also:** cross-topology restore (this doc's own Phase 2 scenario) **fails** — a single-database restore from an `allDatabases` backup demands `artifactFilename`, which such a backup never sets, while `databaseArtifacts` holds the exact filename; the error tells the user to "re-run the backup with a recent operator", which does not help. `Neo4jPlugin` phase flaps Ready↔Installing forever while APOC works. `support-bundle` collects no operator logs though its help text and docs promise them. `explain` does not know the phase `Ready`. Two validator asymmetries: `dbms.mode` is rejected on standalone but accepted on cluster; heap-vs-limit is rejected on cluster but accepted on standalone (demonstrated: a standalone that validated clean crash-looped). Three published examples fail validation (`plugins/standalone-plugin-example.yaml`, `property_sharding/advanced-property-sharding.yaml`, `end-to-end/complete-deployment.yaml`) — **and all 8 of those errors pass `kubectl apply --dry-run=server`, which answers design Q1 empirically: `validate` earns its place.** **Second pass (TLS trio + Phase 3) found one more CLI defect and one doc defect.** `kubectl neo4j cypher` could not connect to ANY TLS deployment: cypher-shell takes trust from the JVM truststore and has no CA flag, so a cert-manager certificate produced only "Failed to establish secured connection with the server". Fixed by building a throwaway truststore in the pod from `/ssl/ca.crt` (falling back to `/ssl/tls.crt`, which is the anchor the operator itself pins when `ca.crt` is absent) rather than dropping to the unverified `+ssc` scheme — and the verification was proven REAL by pointing it at a mismatched certificate and watching it refuse. The Phase 3 sharded-backup row named a field (`spec.target`) that does not exist; corrected above. **TLS trio all pass:** CA path (no `PINNING` line, diagnostics fresh); pinning engages with `pinnedFingerprintSHA256` equal to `openssl`'s and diagnostics still advancing; a same-SAN certificate from another CA is refused with `x509: certificate signed by unknown authority`, and restoring `tls.crt` recovers. **Phase 3 all pass** on `2026.06.0` enterprise with `propertyShardingReady=true`: a `Neo4jShardedDatabase` whose CR name (`test-sharded-db`) differs from its logical name (`testdata`) went Ready with `testdata`, `testdata-g000`, `testdata-p000` and `testdata-p001` all online, and one backup captured all three shard artifacts. **Fixes verified live on a fresh cluster afterwards:** the documented `export` form parses; `cypher -c` returns on a cluster (5/5, zero orphaned shells, writes included); `diagnose Kind/name -n ns` works; `explain Ready` gives guidance; the support bundle carries `operator/<ns>/<pod>/manager.log` (found via the Deployment's selector — the identifying label is on the Deployment, not its pods) with redaction intact; the plugin phase held `Ready` across 12 samples while APOC answered `apoc.version()`, and plugin reconciles fell from ~1/second to 2/minute. **Verified pass:** standalone Ready in 96s from the published URL, database/user/role incl. hyphenated CR-name resolution and all three privileges, APOC, backup→restore round trip, `system` not restorable, alias create/re-point/drift-correction, support-bundle redaction (grepped for all three real passwords, zero hits), cluster formation 3/3 with 3-primary topology, `diagnose` on healthy vs. restarting pods. |
| _(v1.15.0 — network mode across TWO Kind clusters)_ | 2026-09-13 | ✅ the last unverified path in CCDR now works end to end | With `make ccdr-lb` there is no longer a reason to leave this unrun. Two separate Kind clusters, operator in both, MetalLB on the upstream only: the proxy took `172.18.255.200`, the downstream cluster reached it (`nc -z` from a pod in the second cluster), and a `Neo4jReplicaDatabase` with `source.mode: network` and `source.addresses` pointing at the proxy reached `Replicating` "0 transactions behind". `type=replica`/`access=read-only`/`writer=false` on both copies, the seeded rows readable downstream, and a row written to the upstream appeared downstream within 40s — so the live stream works, not just the seed. **Two false alarms worth recording, both mine.** `kind create cluster` switches the current context, so a batch of `kubectl` commands silently targeted the wrong cluster. Then the downstream appeared to be missing the `Neo4jReplicaDatabase`, `Neo4jReplicaPromotion` and `Neo4jDatabaseAlias` controllers in production mode — 23 started, not 26 — which looked exactly like a release blocker. It was the harness: `config/default` renders `ghcr.io/…:latest`, not `controller:latest`, so the sed that was meant to swap in the local build did nothing and the cluster ran a PUBLISHED release predating those controllers. With the right image all 26 start. **If you deploy a second cluster by hand, check the image before believing a missing controller.** |
| _(v1.15.0 — CCDR proxy, first time ever)_ | 2026-09-13 | ✅ the proxy path verified end to end with MetalLB; **one security defect found and fixed** | The gap this doc recorded as "no LoadBalancer-on-Kind tooling today" was hiding a live defect, not just an unverified path. `spec.crossClusterReplication.loadBalancerInternal` defaults to `true` and calls itself the main mitigation against public exposure; it was computed and thrown away — `CCDRProxyLoadBalancerInternalEffective` had no caller outside its own unit test, and no internal-LB annotation existed anywhere in the codebase. On AWS, Azure or GCP that meant a PUBLIC load balancer in front of the transaction-shipping port. Fixed in 9c46aaa, verified live on the Service. **The harness:** `hack/metallb-setup.sh` + `make ccdr-lb`, an L2 pool carved from the top of the `kind` Docker network (`172.18.255.200-250`), which Kind clusters share — so it is reachable from a second Kind cluster with no host routing. **Verified with it:** the proxy Service gets `172.18.255.200`, `status.crossClusterReplication` goes `ready: true` publishing one address per server ordinal (`:16000`, `:16001`), and the servers advertise the proxy endpoint — `server.cluster.advertised_address=172.18.255.200:16000`. None of that had ever been observed. **Also corrected:** the Part D advertised-address row named the wrong file for the second day running; the server runs with `--config-dir=/tmp/neo4j-config`, and `/var/lib/neo4j/conf/neo4j.conf` is the image default the operator never writes. |
| _(v1.15.0 — network mode with TLS ON, two Kind clusters)_ | 2026-09-14 | ✅ the configuration CCDR is actually sold with now works; **two operator bugs found, both release blockers** | Every earlier network-mode run had TLS off. Turning it on — `mode: cert-manager`, `strictPeerValidation: true`, each cluster carrying the other's CA in `spec.tls.additionalClusterTrustCAs` — broke it twice, in ways no unit test could see. **(1) The peer-CA feature could not work at all.** Each CA was mounted as its own volume with a subPath at `/ssl/trusted/peer-ca-<i>.crt`, but `/ssl/trusted/` is content of the certs volume itself (its own `trusted/ca.crt` item), and a volume cannot be mounted inside another volume's content: the kubelet refused the pod with *"not a directory"* and the server crash-looped with exit 128. The certs volume is now a projected volume carrying the cluster's own material plus one `trusted/peer-ca-<i>.crt` item per peer (d36c775) — which is what the API reference already claimed it did. **(2) The fix changed nothing, which was the second bug.** A forming StatefulSet accepted only `hasCriticalTemplateChanges` (image, resources, securityContext, serviceAccount) — not volumes, env or init containers. So a template defect that stops the pods going ready makes every reconcile take the conservative branch, which drops the repair, which keeps the pods from going ready: **a cluster wedged by its own template could not be fixed by editing the CR**, only by deleting the StatefulSet by hand. A forming StatefulSet now also accepts a template whose hash differs from the one the controller last stamped, since that is intent rather than churn (40713d7). **Verified after both fixes:** the corrected volume reached a StatefulSet at 1-of-2 ready, both servers rolled, both clusters went Ready; each side holds the other's CA in `/ssl/trusted/` (md5 of dev's `peer-ca-0.crt` equals DR's own `ca.crt` and vice versa); a `Neo4jReplicaDatabase` in network mode against the proxy at `172.18.255.200:16000` reached `Replicating` "0 transactions behind"; the 3 seeded rows read downstream, a 4th written upstream appeared downstream, and a write to the replica was refused. **Both bugs needed a real two-cluster TLS walk** — the first is a kubelet behaviour, the second only appears when a cluster is *already* failing. **Observation, not fixed:** the replica's `ClusterNotReady` condition is negative-polarity (`status: False` + `reason: ClusterReady` means the cluster IS ready). It is consistent across the role, rolebinding and replica controllers, so it is a naming convention rather than a defect; renaming it is a status-API break and not release-day work. |
| **v1.15.0 release verification** | 2026-09-13 | ✅ Phases 0-3 + TLS trio + Phase 5 pass on current `main`; **one incomplete fix caught and finished** | Full journey on the release candidate, after the day's CCDR and CLI work. **Phase 0** — plugin discovery, exit-code contract, every example directory clean, `--version`, `explain Ready`; `preflight` caught the unschedulable request and the missing StorageClass before apply; `diagnose` named ImagePullBackOff, the unbound PVC (now marked `…` as the consequence it is), the missing class and the scheduling failure; version skew fires when forced. **Phase 1** — standalone Ready from the published raw URL; users/roles incl. hyphenated CR-name resolution; APOC working with the plugin phase holding `Ready` across 8 samples (the flap fix); backup→restore round trip with the marker returning; `cypher -c` writing without hanging; support bundle carrying `operator/<ns>/<pod>/` logs with **zero** hits for either real password. **TLS trio all pass** — CA path clean, pinning with a fingerprint equal to `openssl`'s, a same-SAN rogue certificate refused with `x509: certificate signed by unknown authority` and recovery on restore; the CLI reached a TLS deployment and fell back to `tls.crt` while `ca.crt` was absent. **Phase 2** — 3 servers Available, a 3-primary database online, `cypher` routed correctly on a cluster. **Phase 3** — `propertyShardingReady`, the sharded family online, and a backup by CR name capturing all three shard artifacts. **Phase 5** — `preflight` and the operator both catch a downstream with no `AWS_REGION`, the CR failing immediately with a message naming the variable and the cluster instead of stalling in `Seeding`; adding the variable unblocks it. **The find:** the morning's failure-surface fix was incomplete. It stopped duplicate status writes but not the alternation — `Seeding` was announced before every attempt and `Failed` written after it, so each pass was a genuine change and drove the next reconcile: **1153 failures logged in 45 seconds** against a bucket that did not exist. Announcing `Seeding` only when the previous attempt had not already failed brought that to **0 in a settled minute**, with the phase staying `Failed` and the server's own error in `status.message`. Only running it found that. |
| _(follow-up, v1.15.0 — backup-mode credentials)_ | 2026-09-13 | ✅ backup mode **works**; the gap is documentation, not machinery | Re-tested the open item from 2026-09-12. Backup-mode replication seeds and replicates **as shipped**, provided the downstream cluster carries the bucket credentials in `spec.env` — `AWS_REGION`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, plus `AWS_ENDPOINT_URL_S3` for S3-compatible stores. Verified end to end on `2026.08.1`: all four vars reached the server container, the replica went `Replicating` "0 transactions behind", and both seeded nodes were readable from the MinIO chain with `type=replica`/`access=read-only` on every copy. The seed and pull are performed by the SERVER via the AWS SDK's default chain, which is why the credentials belong there — so this is a **documentation and failure-surface gap, not a missing feature**, and the earlier "cannot work against object storage" reading was too strong. `spec.source.credentialsSecretRef` is still consumed nowhere and is now documented as such on the field itself, in the guide, in the troubleshooting table and in the example. **Still worth fixing:** the CR stays `Seeding` with an optimistic message while the server refuses, and retries a permanent `ArgumentError` about once a second — the error never reaches `status.message`, which is what made this expensive to diagnose. |
| _(pre-release, v1.15.0 — Phase 5 B and C)_ | 2026-09-12 | ⚠️ Parts B and C run on the first real `2026.08.1`; **two operator bugs fixed, one open** | **Three CCDR statements were unusable on the version that introduces the feature.** `CREATE REPLICA DATABASE` and `CALL dbms.promoteReplicaDatabase` are Cypher 25 syntax, and `system` still defaults to Cypher 5 on 2026.08, so the server does not warn — it reports the feature as ABSENT (*"Invalid input 'DATABASE': expected a graph pattern"*; *"There is no procedure with the name … registered"*). Both fixed by prefixing with `CYPHER 25` (aa1fea6, e242a9c) and verified end to end. The operations manual omits the prefix because its examples assume a Cypher 25 default — no unit test can see a parser's opinion, so only running it against a real server could find this. **STILL OPEN — backup-mode replication cannot work against object storage:** the seed and pull are performed by the SERVER, which therefore needs cloud credentials and a region in its own environment. `AWS_REGION` is projected only into backup/restore **Jobs**, never onto the servers, and `Neo4jReplicaDatabase.spec.source.credentialsSecretRef` is declared in the CRD and **read nowhere in the codebase**. Live result: `CREATE REPLICA DATABASE` reaches the server and fails *"Unable to load region from any of the providers"*, while the CR sits at `Seeding` forever with a message that never mentions the error, retried ~1×/second on a permanent `ArgumentError`. **Part C (network mode) passes fully** and needs no object storage: replica `Replicating` "0 transactions behind", `type=replica`/`access=read-only`/`writer=false` on all three copies, data across the wire. **Part B passes on a network-sourced replica**, which exercises the same promotion machinery: write refused (*"database is read only"*) while `--access-mode=read` succeeds; an alias pre-staged against the replica goes `Ready` and, unchanged, accepts writes after promotion; CR-driven promotion converts every copy; deleting a `Promoted` CR leaves the database and its data intact; and **out-of-band promotion is observed, not "corrected"** — promoted by hand at a cypher-shell, the operator reported `Completed`/`Promoted` without dropping the database, which is the failure mode the whole observe-before-act design exists to prevent. `observedLagTxIds` is absent at zero lag by design; the message carries *"0 transactions of lag were made permanent"* and `lastCommittedTxn`. |
| _(pre-release, v1.15.0 — Phase 5 partial)_ | 2026-09-12 | ⚠️ Phase 5: version gate + Parts A and D pass; **Parts B and C still blocked** | **The 2026.08 tags on Docker Hub are EMPTY.** `2026.08-enterprise` and `2026.08.1-enterprise` appeared on 2026-09-11 and are listed by the Hub tags API, but carry no image: `images: []`, `docker manifest inspect` says *"no such manifest"*, and an explicit pull fails for **both** amd64 and arm64. A tag listing is not an image — check `images[]` is non-empty before concluding a version is available. The real pullable ceiling is still **2026.07.1**. **Ran anyway, because only *hosting* a replica needs 2026.08:** scenario 1 — the version gate — fires exactly, `Ready=False(ReplicaVersionTooOld)` with *"requires Neo4j 2026.08 or later on the downstream cluster; ccdr-cluster runs 2026.06-enterprise"*, and it is checked BEFORE a missing credentials Secret, so nothing else proceeds as designed. **Part A all pass** on a 2026.06 upstream: `replicationPullURI` published, all three chain-breaking configs refused with the rule named, a FULL (tx -1→2) then a real differential (tx 3→5) landing as 5.1KiB + 2.0KiB artifacts at exactly the published URI, and `export replica-database` emitting a clean manifest on stdout with notes on stderr that `validate` then accepts. **Part D all pass**: proxy ConfigMap/Deployment/Service render and the cluster still reaches `Ready` with the LoadBalancer stuck `<pending>`; NetworkPolicy goes 3 → 4 ingress rules with the new one scoped to the `ccdr-proxy` pod on 6000 only; the advertised address stays the pod's own; toggling off removes everything and returns the policy to 3 rules. **Three doc rows corrected** (the advertised-address row named the ConfigMap, which never holds the value; Part A gained the same-path guard; the FULL/DIFF row now says where the backup type actually lives). **One CI defect found:** `minio/minio:latest` and `minio/mc:latest` on Docker Hub are no longer anonymously pullable (*pull access denied … insufficient_scope*, on both the host and a Kind node), which would fail the extended MinIO specs; switched to quay.io, where the published example already pointed. **Parts B and C need a real 2026.08+ image and remain unrun.** |
