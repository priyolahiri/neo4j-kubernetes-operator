...
| Advertised address stays internal until ready | `status.crossClusterReplication` stays empty (Kind has no LoadBalancer controller, so the Service never gets an ingress address) → the EFFECTIVE config in the pod still advertises the pod itself: `kubectl exec <pod> -c neo4j -- grep advertised_address /tmp/neo4j-config/neo4j.conf` shows `server.cluster.advertised_address=<pod FQDN>:6000`, not the proxy's hostname. **Two wrong files to avoid.** The ConfigMap does not hold advertised addresses at all (it says so: *"advertised addresses will be set dynamically by startup script"*) — `buildStartupScriptForEnterprise` substitutes them at pod start, overriding 6000 only once `status.crossClusterReplication.LoadBalancerHostname` is set. And `/var/lib/neo4j/conf/neo4j.conf` is the IMAGE's default config, which the operator never writes: the server runs with `--config-dir=/tmp/neo4j-config`, so that is the only file that reflects reality. This row named the ConfigMap until 2026-09-12 and `/var/lib/neo4j/conf` until 2026-09-13; both readings happened to agree with the right answer on plain Kind, where the override is not applied anyway, which is exactly how a wrong instruction survives |
...
| The proxy gets a real address | `status.crossClusterReplication.ready: true` with one address per server ordinal (`<ip>:16000`, `:16001`), and `server.cluster.advertised_address=<ip>:16000` in `/tmp/neo4j-config/neo4j.conf`. The override applies only once the Service has an ingress address, so this is the first place it is observable at all |
