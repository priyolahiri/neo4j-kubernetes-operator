...
| Advertised address stays internal until ready | `status.crossClusterReplication` stays empty (Kind has no LoadBalancer controller, so the Service never gets an ingress address) → the EFFECTIVE config in the pod still advertises the pod itself: `kubectl exec <pod> -c neo4j -- grep advertised_address /var/lib/neo4j/conf/neo4j.conf` shows `server.cluster.advertised_address=<pod>:6000`, not the proxy's hostname. **Do not look in the ConfigMap** — advertised addresses are not in it (it says so: *"advertised addresses will be set dynamically by startup script"*); `buildStartupScriptForEnterprise` substitutes them at pod start, and only overrides 6000 once `status.crossClusterReplication.LoadBalancerHostname` is set. This row said "in the rendered ConfigMap" until 2026-09-12, which sends the runner to an artifact that never contains the value |
