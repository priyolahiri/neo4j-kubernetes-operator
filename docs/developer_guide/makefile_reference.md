...
**Description**: Regenerate the `artifacthub.io/crds` annotation in `Chart.yaml` from `config/crd/bases/`. Each CRD's user-facing description lives in a curated mapping in [`scripts/helm-sync-artifacthub-crds.sh`](https://github.com/priyolahiri/neo4j-kubernetes-operator/blob/main/scripts/helm-sync-artifacthub-crds.sh) — the script exits non-zero if a CRD has no description, so adding a new CRD without updating the description map is impossible.
