...
> **Source-of-truth table**: see the *Generated artifacts* section in [`CLAUDE.md`](../../CLAUDE.md) for a full mapping of generated files to their sources.
...
**Description**: Regenerate the `artifacthub.io/crds` annotation in `Chart.yaml` from `config/crd/bases/`. Each CRD's user-facing description lives in a curated mapping in [`scripts/helm-sync-artifacthub-crds.sh`](../../scripts/helm-sync-artifacthub-crds.sh) — the script exits non-zero if a CRD has no description, so adding a new CRD without updating the description map is impossible.
