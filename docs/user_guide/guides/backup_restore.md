...
kubectl apply -f https://raw.githubusercontent.com/priyolahiri/neo4j-kubernetes-operator/main/examples/backup-restore/backup-pvc-simple.yaml
...
> **Backups are never deleted by deleting the `Neo4jBackup` CR.** Reclaim backup storage explicitly with `kubectl delete pvc`. One PVC can be shared across multiple Neo4jBackup CRs (e.g. a daily + a weekly schedule writing to the same volume).
