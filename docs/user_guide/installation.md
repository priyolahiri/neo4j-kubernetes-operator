...
kubectl apply --server-side -f https://github.com/priyolahiri/neo4j-kubernetes-operator/releases/download/<version>/neo4j-kubernetes-operator.yaml
...
Helm 3.8 or later is required for OCI support.
...
Use the chart version without the `v` prefix (for example, `1.16.0`).
...
```bash
RELEASE_VERSION=v1.16.0  # Replace with desired version
...
```bash
kubectl apply --server-side -f https://github.com/priyolahiri/neo4j-kubernetes-operator/releases/download/${RELEASE_VERSION}/neo4j-kubernetes-operator.yaml
```
...
```bash
RELEASE_VERSION=v1.16.0  # The version you are upgrading to
...
helm install neo4j-operator ./charts/neo4j-operator \
  --namespace neo4j-operator-system \
  --create-namespace \
  --set image.tag="${LATEST_TAG#v}"   # e.g. 1.16.0 — see note below
```
...
```bash
RELEASE_VERSION=v1.16.0  # Replace with your installed version
