...
Composite databases give one query endpoint over several constituent databases (`USE cineasts.latest`). Runnable manifests are in `examples/composite-databases/` — a local composite, granting access through one, and remote constituents in both authentication modes. See the [Composite databases guide](guides/composite_databases.md).
