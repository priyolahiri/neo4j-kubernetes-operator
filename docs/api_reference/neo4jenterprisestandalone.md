...
#### `acceptLicenseAgreement` (string)
Your explicit acceptance of the Neo4j Enterprise license. **Required** — the
operator refuses to deploy until it is set.
...
This operator is **independent** and does **not** provide or accept a Neo4j
license on your behalf — you must hold your own valid Neo4j Enterprise license.

```yaml
acceptLicenseAgreement: "eval"   # "yes" (commercial) or "eval" (30-day evaluation)
```
...
env:
  - name: NEO4J_dbms_jvm_additional
    value: "-XX:+UseG1GC"
  # Note: NEO4J_ACCEPT_LICENSE_AGREEMENT and NEO4J_AUTH are operator-managed and
  # ignored here — set spec.acceptLicenseAgreement and spec.auth instead.
  # Use the Neo4jPlugin CRD for plugin management instead of NEO4J_PLUGINS.
```
