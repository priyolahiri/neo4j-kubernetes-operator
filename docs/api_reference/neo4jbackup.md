...
| `mode` | `string` | ❌ | `standard` (default) or `replication-source`. `replication-source` marks this backup as the differential chain a cross-cluster replica pulls from, and enforces the constraints that keep that chain usable: single-database scope, no operator-side `retention`, no competing `Neo4jBackup` writing to the same storage location, and a required `schedule`. It also publishes `status.replicationPullURI`. **Limit:** for cloud storage the operator never prunes — retention is delegated to bucket lifecycle rules it cannot read — so a lifecycle rule expiring objects in this directory will silently break the chain and force a replica rebuild. Exclude the path from lifecycle expiry. See the [Cross-Cluster Replication guide](../user_guide/guides/cross_cluster_replication.md). |
