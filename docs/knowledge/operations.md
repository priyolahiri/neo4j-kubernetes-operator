...
- **rule:** `spec.auraFleetManagement.provision` adds a **Phase 0** in front of the existing two fleet phases, and all three stay separate: (0) register a Fleet Manager deployment + mint its token into a Secret, (1) merge `fleet-management` into `NEO4J_PLUGINS`, (2) once Ready, `CALL fleetManagement.registerToken($token)`. `provision` and `tokenSecretRef` are **mutually exclusive**. Six landmines:
  ...
  3. `POST .../token` **400s when the deployment already has a token** — decide create-vs-rotate from `GetDeployment`, never by trying POST and handling the failure.
  4. Deliberate refusals here are marked with `refusef` / `isFleetRefusal` and checked BEFORE `aura.IsTransient`, because it reports every non-`APIError` as retryable — **see id 88**, the single home for that trap, which applies to all Aura reconcilers.
...
- **enforcement:** unit test + CEL (`provision` XOR `tokenSecretRef`, credentials XOR, name cap, both enums) + inline validator (`validateAuraFleetProvision`) + **PROSE-ONLY — at risk** for the undocumented request bodies. Read paths were **verified against a live Aura account on 2026-07-30**; the WRITE paths (deployment create, token create/rotate/delete) remain **UNVERIFIED — known gap**, since verifying them means mutating a real fleet.
...
### id 87 — Where the live Aura API disagrees with its own published spec (verified 2026-07-30)
...
- **rule:** The published v2beta1 spec is **not** a reliable description of the live API. Where they differ, **LIVE WINS** — do not "correct" the code back to the spec. Observed differences, all confirmed against a real account:
  1. **`GET .../fleet-manager/deployments/{id}` is `{"data":…}`-wrapped**, though the spec declares it bare (`allOf:[DetailedDeployment,{}]`). Decoding it bare yields an all-zero struct **with no error**, which is the worst kind of failure.
