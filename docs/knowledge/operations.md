...
- **rule:** Aura resources are managed via **Aura-native, API-driven CRDs on the v2beta1 API — NEVER by pointing `Neo4jDatabase`/`Neo4jUser`/`Neo4jRole`/`Neo4jRoleBinding` at an Aura instance** (that Cypher-over-Bolt path was removed; those CRDs are cluster/standalone-only via `clusterRef`). Two landmines the spec pins: (1) database endpoints are **instance-nested** (`/organizations/{org}/projects/{project}/instances/{instance}/databases`, built by `dbCollectionPath`) and, unlike ip-filters, **ARE wrapped in the standard `{"data":…}` envelope** (unwrapped by `doV2Data`); Aura owns database topology per tier, so `AuraDatabase` has **no topology field**. (2) v2beta1 "users" are **console/org/project identity** (`ORG_*`/`PROJECT_*`/`METRICS_READER` roles, granted by **inviting an email**), NOT in-database Neo4j users — there is no API to manage a DBMS user inside an Aura instance. The `AuraDatabase`/invite/member **request bodies are not fully schema'd upstream** and are mirrored from the documented response shapes — BETA/best-effort.
- **why:** Aura rejects the full `CREATE DATABASE` grammar over Bolt (topology/seed are Aura-managed) and exposes no in-DB user API, so the Cypher path silently supported only a subset and failed opaquely; API-driven CRDs model exactly what Aura offers. Bodies are best-effort because v2beta1 can break without a version bump.
- **pinned-by:** `TestDatabaseLifecycle`, `TestDatabaseDeleteTreats404AsSuccess` (`internal/aura/database_v2beta1_test.go`) pin the instance-nested path + data envelope; `TestMembersAndInvites` (`internal/aura/members_v2beta1_test.go`) pins the org/project user + invites paths + envelope. They assert the client matches the *published schema*, NOT a live account — **no live-API contract test — known gap** pending a validated account.
- **enforcement:** unit test (matches published v2beta1 schema) + **PROSE-ONLY — at risk** for live-account drift and the undocumented create/invite bodies.
