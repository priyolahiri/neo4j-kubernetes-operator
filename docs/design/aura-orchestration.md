...
- **Phase 3 (done — the non-API-gated items, v1 + Bolt):** Aura-target-aware `Neo4jDatabase`/`Neo4jUser`/`Neo4jRole`/`Neo4jRoleBinding` via a new `auraInstanceRef` (CEL exactly-one-of with `clusterRef`); a `NewClientForAura` Bolt client + a `ResolvedTarget` Aura arm + a shared `targetRefKey` so auth-model cross-references stay scoped to the same Aura target; a fail-fast multi-DB tier gate for `Neo4jDatabase`. This delivers "multi-DB on Aura" over Cypher, without v2beta1.
...
- **Phase 3 (still deferred):** `AuraDatabase` (redundant with Aura-target `Neo4jDatabase` on multi-DB tiers), org/project RBAC, billing — all v2beta1-only; hold on stability, not availability. `AuraInstanceClass` remains a non-API-gated operator-side follow-up.
