...
**Deferred (Phase 3):** v2beta1-only features — multi-database (`AuraInstance.spec.multiDatabase` + `AuraDatabase`), IP filters, org/project RBAC, billing/usage.
...
- **Phase 3 (superseded — Cypher-over-Bolt Aura targeting, REVERTED):** an earlier cut added `auraInstanceRef` to `Neo4jDatabase`/`Neo4jUser`/`Neo4jRole`/`Neo4jRoleBinding` (CEL exactly-one-of with `clusterRef`) with a `NewClientForAura` Bolt client and a fail-fast multi-DB tier gate. This was **reverted**: Aura does not accept the full `CREATE DATABASE` grammar over Bolt (topology/seed are Aura-managed, not user-settable), and in-database user/role management has no Aura equivalent (Aura governs access via console-RBAC, a different concept). The self-managed CRDs are now cluster/standalone-only, and Aura databases/access are modelled by **Aura-native, API-driven CRDs** on the v2beta1 API — see below.
- **Phase 3 (done — first v2beta1 item, BETA/best-effort):** `AuraIPFilter` (network allowlist) on the v2beta1 API, in an isolated, clearly-flagged client (`internal/aura/ipfilter_v2beta1.go`). The client shape follows the **official v2beta1 `IpFilter` schema**: filters are **organization-scoped** (`/organizations/{org}/ip-filters`, built by `orgIPFilterPath`), the ip-filter endpoints are the one v2beta1 resource returned **without a `{data:…}` envelope** (`doV2JSON`), and the allowlist is `allow_list:[{address,prefix_len,description}]` applied to instances via `filtered_entities.instances`. Still BETA/best-effort — v2beta1 may break without a version bump. See `docs/knowledge/operations.md`.
- **Phase 3 (in progress — Aura-native v2beta1 CRDs, BETA/best-effort):** `AuraDatabase` (+ per-DB backup/restore) on `…/instances/{id}/databases`, and console-RBAC (`AuraOrganizationMember`/`AuraProjectMember`/`AuraInvite`) on `…/organizations/{org}/users` + `/invites`. These replace the reverted Cypher-over-Bolt path. The `AuraDatabase` create body is undocumented upstream → mirrored from the response shape, BETA/best-effort — **confirmed correct by walking the write paths live on 2026-07-31** (create → backup → restore → delete). Still deferred: billing (surface as metrics, not a CRD), `AuraServiceAccount` (no `/service-accounts` path in the current v2beta1 spec), and `AuraInstanceClass` (non-API-gated operator-side follow-up).
- **Multi-database is an INSTANCE property, not a database-CRD feature** (learned from that live walk; `docs/knowledge/operations.md` id 90). `multi_database` is fixed when the instance is created and exists only in the v2beta1 instance API, so the `AuraDatabase` family is unusable against anything v1 created — the tier is not the control. `AuraInstance.spec.multiDatabase` therefore routes the *create* to v2beta1 (`internal/aura/instance_v2beta1.go`) and keeps managing through v1 afterwards. The verdict is reported as three-valued in `status.atProvider.multiDatabase` because the v2beta1 instance GET returns HTTP 500 for v1-created instances, so "unknown" is a real state that must not collapse to "false".
