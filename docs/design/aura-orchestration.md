...
- **Phase 3 (done — first v2beta1 item, BETA/best-effort):** `AuraIPFilter` (network CIDR allowlist) on the v2beta1 org/project API, in an isolated, clearly-flagged client (`internal/aura/ipfilter_v2beta1.go`) whose exact contract is **reconstructed and unvalidated pending a live check** (single-point-of-correction via `ipFilterCollectionPath` + `v2beta1Envelope`). See `docs/knowledge/operations.md`.
