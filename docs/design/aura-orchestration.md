...
- **Phase 3 (in progress — Aura-native v2beta1 CRDs, BETA/best-effort):** `AuraDatabase` (+ per-DB backup/restore) on `…/instances/{id}/databases`, and console-RBAC (`AuraOrganizationMember`/`AuraProjectMember`/`AuraInvite`) on `…/organizations/{org}/users` + `/invites`. These replace the reverted Cypher-over-Bolt path. The `AuraDatabase` create body is undocumented upstream → mirrored from the response shape, BETA/best-effort. Still deferred: billing (surface as metrics, not a CRD), `AuraServiceAccount` (no `/service-accounts` path in the current v2beta1 spec), and `AuraInstanceClass` (non-API-gated operator-side follow-up).
