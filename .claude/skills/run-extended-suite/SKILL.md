...
- **Core, per-PR**: `.github/workflows/integration.yml` ("Integration Tests")
  ...
  CalVer (`2026.08.1-enterprise`), in parallel. Auto-triggered on PRs touching
  runtime paths.
...
1. **CI — dispatch the Extended workflow and watch it to completion.** This is
   ...
   ```
   Defaults to the pinned anchor CalVer (`2026.08.1-enterprise`) with a 150-minute
...
2. **Local — bring up the Kind test cluster + operator once**, then run the
   ...
   ```bash
   export NEO4J_VERSION=2026.08.1-enterprise # or 5.26-enterprise for the LTS floor
