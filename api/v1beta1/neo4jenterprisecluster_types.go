...
type Neo4jEnterpriseClusterSpec struct {
	...
	// AcceptLicenseAgreement is your explicit acceptance of the Neo4j Enterprise
	// license. Neo4j Enterprise is commercial software owned by Neo4j, Inc.; this
	// operator is independent and does not provide or accept that license on your
	// behalf — you must hold your own valid license. Set "yes" to accept the
	// commercial license agreement, or "eval" to accept the 30-day evaluation
	// license. Required: the operator refuses to deploy until you set it.
	// +kubebuilder:validation:Enum=yes;eval
	// +optional
	AcceptLicenseAgreement string `json:"acceptLicenseAgreement,omitempty"`
...
//
// Requires spec.tls.mode=cert-manager. The proxy is a TCP passthrough: it
// terminates nothing and authenticates nothing, so the cluster SSL policy is
// the only access control in front of the port it publishes. Enabling this
// without TLS is rejected.
type CrossClusterReplicationSpec struct {
	...
	//
	// Two things to weigh before turning it on. Security: the exposed port is
	// protected only by the cluster SSL policy, so spec.tls.mode=cert-manager
	// is required, strictPeerValidation must stay true (the default), and the
	// peer CAs in spec.tls.additionalClusterTrustCAs are the access-control
	// list for that port. Cost: intra-cluster secondary catch-up traffic
	// shares port 6000 with cross-cluster replica traffic, so it routes
	// through the load balancer too, adding latency and LB cost proportional
	// to write volume rather than to replication volume.
	// +optional
	Enabled bool `json:"enabled,omitempty"`

	// LoadBalancerInternal keeps the proxy's Service on a private/internal
	// load balancer where the cloud provider supports it. Default true, and
	// worth leaving true: this design has no DNS split to keep intra-cluster
	// traffic off the load balancer, so an internal LB is the main network
	// boundary available. Set it false only when the downstream cluster is
	// genuinely outside the VPC/VNet.
	//
