...
func (r *Neo4jEnterpriseStandaloneReconciler) buildEnvVars(standalone *neo4jv1beta1.Neo4jEnterpriseStandalone) []corev1.EnvVar {
	...
	envVars = append(envVars, corev1.EnvVar{
		// User's explicit license acceptance ("yes" or "eval"), required +
	...
	// and NEO4J_AUTH is managed from the admin Secret.
	for _, e := range standalone.Spec.Env {
		if e.Name == "NEO4J_ACCEPT_LICENSE_AGREEMENT" || e.Name == "NEO4J_AUTH" {
			continue
		}
